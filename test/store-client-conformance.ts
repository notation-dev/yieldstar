...
export type StoreConformanceBackend = {
  name: string;
...
/**
 * Build the sqlite-specific inputs for registerStoreClientConformance: a
 * backend that opens a fresh database per harness, plus a database factory
 * for the sqlite-only suites. `name` keeps each runner's stores distinct.
 */
export function sqliteConformance(name: string, createSqliteDb: () => SqliteDriver) {
  const sqliteBackend: StoreConformanceBackend = {
    name,
...
const backends: StoreConformanceBackend[] = [
  {
    name: "memory",
    create(schedulerClient: SchedulerClient) {
  ...
  params.sqliteBackend,
];

for (const { name, create } of backends) {
describe(`${name} store client`, () => {
