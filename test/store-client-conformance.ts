...
export function defineStoreClientConformance(target: StoreClientTarget) {
  const Store = defineStore(`${target.name}-store-conformance`, schema<State>());
  ...
  describe(`${target.name} store client`, () => {
    ...
    test("store listing and deletion preserve logical instance semantics", async () => {
      const harness = await target.create(noopScheduler());
      const OtherStore = defineStore(`${target.name}-other-conformance`, schema<State>());
      try {
...
export function defineWakeDeliveryConformance(target: StoreClientTarget) {
  const Store = defineStore(`${target.name}-wake-conformance`, schema<State>());
...
export function defineSharedBackendConformance(target: SharedStoreClientTarget) {
  const Store = defineStore(`${target.name}-shared-conformance`, schema<State>());

  describe(`${target.name} shared backend`, () => {
...
export function defineDurableWakeConformance(target: DurableStoreClientTarget) {
  const Store = defineStore(`${target.name}-durable-wake-conformance`, schema<State>());

  describe(`${target.name} durable wake delivery`, () => {
