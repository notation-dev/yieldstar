...
The child process creates a `WorkflowRunner` using the same SQLite file. On every invocation it replays cached steps, executes until the next durable boundary, sends a result when complete, and exits. Delayed and suspended workflows resume later in a new child.
