...
# Repeatable Workflow Store Handles

## Abstract

`step.store` treats obtaining a store handle — the value used to read or change a store — as a durable step. Consequently, a loop or helper that asks for the same store is rejected unless every caller shares the first handle or coordinates a cache.

Obtaining a store handle, however, does not need to be a step at all. Treating handles as ordinary repeatable values simplifies the API and leaves durability only on store creation, reads, and writes.
...
Return a store handle immediately each time it is requested, and move durable initialization to an explicit `getOrCreate` operation:
...
If `id` is omitted, `step.store` uses `event.executionId`.

## Required Semantics

- Store identity remains `definition.name + id`.
- Asking for a handle does not read, create, or change the store and records no workflow step.
- Repeated requests for the same identity return handles for the same store.
- Reads, writes, and `getOrCreate` remain durable operations with explicit step keys.
- `getOrCreate` evaluates `initial` only when the store does not exist.
- Concurrent or repeated initialization records one initial state and returns that store to every caller.
...
This RFC does not duplicate creation on `RuntimeStore`. Code outside a workflow can use the public `StoreClient` method.

## Acceptance Histories

An implementation must explain these histories:

1. A loop asks for the same project handle on several turns. Handle access never creates a repeated step-key error.
2. Two executions initialize one store at the same time. Exactly one initial state is recorded.
