...
The application can repeatedly check the store, which delays responses and repeatedly runs code when nothing has changed. Alternatively, it can start a second workflow for the timer and store whether the condition or timeout won, solely to prevent both workflows from acting.
...
// The condition and deadline can both finish this step, but only the first result is kept.
...
The existing form without a deadline remains unchanged; only a timed wait returns a result with a `status`.
...
- The store condition and deadline compete to complete one durable step.
- The first recorded result is the only result returned to workflow code.
- Starting the wait cannot miss a store change or deadline that happens at the same time.
- After one result wins, the remaining condition or timer cannot run workflow code and is eventually removed.
- The deadline cannot win before its timestamp; the runtime does not promise to resume the workflow immediately.
...
2. The deadline finishes the wait first. A later matching store update cannot replace the timeout result.
3. A matching store change happens at the deadline. One durable result wins, and a restart returns the same result.
4. The worker crashes while the wait is being created or resolved. Recovery neither loses an outcome nor changes a recorded winner.

## Not Specified

This RFC does not choose how the wait and timer are stored, how the runtime avoids missed changes, how ties are ordered beyond the first recorded result, or when cleanup runs.
